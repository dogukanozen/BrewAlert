...
**Read [`AGENT.md`](AGENT.md) first.** It is the single source of truth for every agent (Codex, Gemini, Claude, …). This file exists only so Codex picks up the brief via its default entry point.

Commit messages **MUST** follow [`docs/commit-style.md`](docs/commit-style.md). No `minor.` / `Update stuff` / bare `wip` commits.

Codex-specific notes (only when they differ from `AGENT.md`):
